...
export type Mastery = 'Introduced' | 'Developing' | 'Reliable' | 'Automatic';
...
export function summarizeSkills(attempts: Attempt[]): SkillSummary[] {
  return [...new Set(attempts.map(item => item.exercise))].map(exercise => {
    ...
    const mastery: Mastery = evidence.length >= 20 && accuracy >= .9 && medianLatencyMs < 2500 ? 'Automatic' : evidence.length >= 12 && accuracy >= .8 ? 'Reliable' : evidence.length >= 4 ? 'Developing' : 'Introduced';
