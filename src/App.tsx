...
export default function App() {
  ...
  function submit(response: string) {
    ...
    attemptStore.add({
      ...
      difficulty: { rootPool: config.rootPool, inversions: config.inversions, register: config.register, timbre: config.timbre, presentation: config.melodic ? 'melodic' : 'harmonic', vocabulary: config.vocabulary ?? 'diatonic' }, replayCount: replays,
  ...
  const play = () => {
    ...
    const melodic = stimulus.melodic ?? (config.kind === 'interval' && config.melodic);
    void audio.play(stimulus.notes.map(pitch), config.kind === 'interval' ? .55 : melodic ? .5 : 1.15, melodic, config.timbre);
  };
