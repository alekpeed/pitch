...
type Page = 'Daily' | 'Diagnostic' | 'Practice' | 'Harmony' | 'Performance' | 'Perform' | 'Harmonize' | 'Transcribe' | 'Transcription' | 'Singing' | 'Explore' | 'Progress' | 'Settings';
const DRILL_KINDS: ExerciseKind[] = [...RECOGNITION_KINDS];
const CATALOG = DRILL_KINDS.map(kind => `${kind}-recognition`);
const PRODUCTION = ['exact-voicing-copy', 'guide-tone-voice-leading', 'scale-degree-production', 'interval-production', 'harmonization', 'reharmonization',
...
const TRANSFER = ['transcribe-melody', 'transcribe-chords', 'transcription'];
...
export default function App() {
  ...
  const harmony = useMemo(() => generateHarmony(seed, harmonyMode), [seed, harmonyMode]);
  void historyVersion;
  ...
  return <div className="app"><aside><div className="brand"><span>PE</span><div>Perfect Ear<small>Musicianship studio</small></div></div><nav>{(['Daily', 'Diagnostic', 'Practice', 'Harmony', 'Performance', 'Perform', 'Harmonize', 'Transcribe', 'Transcription', 'Singing', 'Explore', 'Progress', 'Settings'] as Page[]).map(item => <button className={page === item ? 'active' : ''} onClick={() => { setPage(item); setAnswer(undefined); }} key={item}>{item}</button>)}</nav><div className="profile"><b>Local profile</b><small>Private · on this device</small></div></aside><main><header><div><p className="eyebrow">{plan.length ? `DAILY SESSION · ${planIndex + 1} OF ${plan.length}` : page === 'Practice' || page === 'Harmony' || page === 'Performance' || page === 'Transcription' ? 'TARGETED PRACTICE' : 'YOUR STUDIO'}</p><h1>{page}</h1></div><div className="session">{attempts.length} attempts recorded</div></header>
